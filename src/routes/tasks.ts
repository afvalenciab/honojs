...
tasks.use(validateJwt);
tasks.use(requireRole("ADMIN"));
...
tasks.get("/:id", validateSchema("param", IdParamSchema), (c) => {
...
tasks.post("/", validateSchema("json", CreateTaskSchema), (c) => {
...
tasks.delete("/:id", validateSchema("param", IdParamSchema), (c) => {
  const { id } = c.req.valid("param");
